...
Scored against the capstone rubric, with a pointer to the evidence for each
criterion. **Total: 21 / 21** on the core + best-practices criteria (cloud
deployment / extra bonus points left to the reviewer's discretion, see below).

| Criterion | Score | Evidence / where to find it |
|---|---|---|
| Problem description | **2 / 2** | [Problem statement](#problem-statement) — clear who has the problem, why, and what the assistant solves |
| Retrieval flow | **2 / 2** | Knowledge base **and** LLM: agentic RAG in n8n over pgvector — [rag-agent.json](n8n/workflows/rag-agent.json), [The RAG flow](#the-rag-flow-agentic) |
| Retrieval evaluation | **2 / 2** | **Three** approaches compared, best one used — [notebook 2](notebooks/2-retrieval-evaluation.ipynb): full-text vs vector vs hybrid+RRF ([results](#retrieval-evaluation)) |
| LLM evaluation | **2 / 2** | **Multiple** approaches compared, best one used — [notebook 3](notebooks/3-rag-evaluation.ipynb): 2 prompt variants judged by an LLM (+ query rewriting) ([results](#rag-evaluation-llm-as-a-judge)) |
| Interface | **2 / 2** | UI **and** API: [Streamlit chat](streamlit/app.py) + REST API (n8n webhook) — [How to run](#how-to-run) |
| Ingestion pipeline | **2 / 2** | Automated: python scripts ([ingestion/](ingestion/)) + n8n ingestion workflow (schedule + webhook) — [Ingestion pipeline](#ingestion-pipeline) |
| Monitoring | **2 / 2** | User feedback (👍/👎) **and** a Grafana dashboard with **7 panels** — [docs-assistant.json](grafana/dashboards/docs-assistant.json), [Monitoring](#monitoring) |
| Containerization | **2 / 2** | Everything (Postgres, n8n, Streamlit, Grafana) in one [docker-compose.yaml](docker-compose.yaml) |
| Reproducibility | **2 / 2** | Clear [How to run](#how-to-run); dataset accessible (download script + committed `chunks.json` / `ground-truth.csv`); all versions pinned (images, python deps in [uv.lock](uv.lock), docs commit) |
| Best practices | **3 / 3** | Hybrid search ✓ · document re-ranking (RRF) ✓ · query rewriting ✓ — all evaluated in [notebooks 2–3](notebooks/) ([Best practices](#best-practices-rubric-extras)) |
| Bonus — cloud deployment | **0 / 2** | Not deployed; compose-based so it's straightforward — see [Limitations & next steps](#limitations--next-steps) |
| Bonus — extra | *reviewer's discretion* | The entire RAG runtime is built in **n8n** (agentic RAG in a visual workflow tool); async LLM-judge adds live quality monitoring at zero added latency; workflows + credentials auto-imported from `.env` on first start |

<sub>Rubric per criterion: 0 = missing · 1 = partial · 2 = full. Best practices are 1 point each (hybrid / re-ranking / query rewriting).</sub>
...
1. **Webhook** receives `{"question": "...", "history": [...]}` (the chat UI
   sends the recent turns in `history`; it is optional)
2. A **Code node** validates the input, folds any `history` into the prompt so
   the agent can resolve follow-ups, and starts a latency timer
